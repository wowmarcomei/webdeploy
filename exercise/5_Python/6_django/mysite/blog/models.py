...
class Article(models.Model):
    ...
    # verbose_name），主要用于 django 的后台系统，不多做介绍。max_length 表示能存储的
    # 字符串的最大长度
    ...
    # STATUS_CHOICES，field 的 choices 参数需要的值，choices选项会使该field在
    # 被渲染成form时被渲染为一个select组件，这里我定义了两个状态，
    # 一个是Draft（草稿），一个是Published（已发布），select组件会有两个选项：
    # Draft 和 Published。但是存储在数据库中的值分别 是'd'和'p'，这就是 choices的作用。
    ...
    class Meta:
        # Meta 包含一系列选项，这里的 ordering 表示排序，- 号表示逆序。即当从数据库中
        # 取出文章时，其是按文章最后一次修改时间逆序排列的。
